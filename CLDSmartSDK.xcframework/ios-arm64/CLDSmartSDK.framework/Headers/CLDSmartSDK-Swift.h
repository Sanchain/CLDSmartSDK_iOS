...
#ifndef CLDSMARTSDK_SWIFT_H
#define CLDSMARTSDK_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="CLDSmartSDK",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
@class NSString;
enum CLDBLEStatusSyncUpdateKind : NSInteger;
@class NSData;
...
enum CldSmartEngineVideoState : NSInteger;
enum CldSmartEngineRecordState : NSInteger;
@class APBLEDevice;
@class APBLEResponse;
SWIFT_PROTOCOL("_TtP11CLDSmartSDK22CLDSmartEngineDelegate_")
@protocol CLDSmartEngineDelegate
@optional
/// Media
/// 加入channel
- (void)onMediaDidConnectedWithUid:(NSUInteger)uid;
/// 离开channel
- (void)onMediaDidDisconnectedWithUid:(NSUInteger)uid;
/// 视频码率数据回调
/// \param bitrate 单位是比特【bit】
///
- (void)onMediaDidVideoBitrateUpdateWithBitrate:(NSUInteger)bitrate;
/// 视频状态变化
- (void)onMediaDidVideoStateChangedWithState:(enum CldSmartEngineVideoState)state;
/// 录像时长变化
- (void)onMediaDidRecordDurationUpdateWithDuration:(NSTimeInterval)duration;
/// 录像状态变化
- (void)onMediaDidRecordStateChangedWithState:(enum CldSmartEngineRecordState)state;
/// 已收到截图
/// \param filePath 截图文件路径
///
/// \param errorCode 截图错误码  等于0：成功  小于0：失败
///
- (void)onMediaDidTakeSnapshotWithFilePath:(NSString * _Nonnull)filePath errorCode:(NSInteger)errorCode;
/// Bluetooth（可能用不到）
/// 蓝牙的状态改变
- (void)onBleDidStateChangedWithState:(CBManagerState)state;
/// 蓝牙已扫描到设备回调
- (void)onBleDidDiscoverWithDevice:(APBLEDevice * _Nonnull)device;
/// 蓝牙已连接回调
- (void)onBleDidConnectedWithState:(APBLEDeviceConnectStatus)state;
/// 蓝牙已断开连接回调
- (void)onBleDidDisconnectedWithError:(NSError * _Nullable)error;
/// 蓝牙已接收到指令数据
- (void)onBleDidReceivedReponseWithResponse:(APBLEResponse * _Nonnull)response;
@end

/// 局域网发现的 UDP 设备模型
SWIFT_CLASS("_TtC11CLDSmartSDK12CLDUDPDevice")
@interface CLDUDPDevice : NSObject
...
- (nonnull instancetype)initWithMac:(NSString * _Nonnull)mac pairingCode:(NSString * _Nonnull)pairingCode matchNum:(NSInteger)matchNum pid:(NSString * _Nonnull)pid deviceName:(NSString * _Nonnull)deviceName OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end

SWIFT_CLASS("_TtC11CLDSmartSDK14CldSmartEngine")
@interface CldSmartEngine : NSObject
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end

typedef SWIFT_ENUM(NSInteger, CldSmartEngineRecordState, open) {
  CldSmartEngineRecordStateStarted = 0,
  CldSmartEngineRecordStateOverDurationError = 1,
  CldSmartEngineRecordStateOtherError = 2,
  CldSmartEngineRecordStateStopped = 3,
};

typedef SWIFT_ENUM(NSInteger, CldSmartEngineVideoState, open) {
  CldSmartEngineVideoStateStarting = 0,
  CldSmartEngineVideoStateStreaming = 1,
  CldSmartEngineVideoStateStopped = 2,
};

@class UIView;
@class NSCoder;
SWIFT_CLASS("_TtC11CLDSmartSDK16FWLiveScrollView")
@interface FWLiveScrollView : UIScrollView <UIScrollViewDelegate>
- (void)layoutSubviews;
...
SWIFT_CLASS("_TtC11CLDSmartSDK8MQTTCore")
@interface MQTTCore : NSObject
/// 开始连接
- (void)startConnect;
/// 断开连接
- (void)disconnect;
...
@interface MediaCore (SWIFT_EXTENSION(CLDSmartSDK)) <AgoraMediaRecorderDelegate>
...
@interface MediaCore (SWIFT_EXTENSION(CLDSmartSDK)) <AgoraRtcEngineDelegate>
...
typedef SWIFT_ENUM(NSInteger, NetworkAPI, open) {
  NetworkAPIGetDeviceList = 0,
  NetworkAPIGetDeviceInfo = 1,
  NetworkAPIGetProductList = 2,
  NetworkAPIAccountRegister = 3,
  NetworkAPIRegistrationValid = 4,
  NetworkAPILoginByPassword = 5,
  NetworkAPIRefreshAccessToken = 6,
  NetworkAPIForgetCheck = 7,
  NetworkAPIForgetValid = 8,
  NetworkAPIResetPassword = 9,
  NetworkAPIMemberPassword = 10,
  NetworkAPIChangePassword = 11,
  NetworkAPIInitSDK = 12,
  NetworkAPILogin = 13,
  NetworkAPILogout = 14,
  NetworkAPIDeleteAccount = 15,
  NetworkAPIPictureCode = 16,
  NetworkAPIImgCode = 17,
  NetworkAPIDeleteDevice = 18,
  NetworkAPIDeviceKeyList = 19,
  NetworkAPIRenameDevice = 20,
  NetworkAPIDeleteDeviceKey = 21,
  NetworkAPIAddDeviceKey = 22,
  NetworkAPIRenameDeviceKey = 23,
  NetworkAPISwitchNotify = 24,
  NetworkAPIGetNotifySwitches = 25,
  NetworkAPIRefreshAPNsToken = 26,
  NetworkAPIBreakKeyInput = 27,
  NetworkAPISyncDeviceAttributes = 28,
  NetworkAPIBindBLEDevice = 29,
  NetworkAPIGetChangeOwnerCode = 30,
  NetworkAPIVerifyChangeOwner = 31,
  NetworkAPIGetNetConfigToken = 32,
  NetworkAPIGetShareUserList = 33,
  NetworkAPIUpdateShareUserList = 34,
  NetworkAPICheckShareUser = 35,
  NetworkAPIGetMQTTParam = 36,
  NetworkAPIWebRtcConfig = 37,
  NetworkAPIBindAccessory = 38,
  NetworkAPIUnbindAccessory = 39,
  NetworkAPIGetBindAccessories = 40,
  NetworkAPIGetUnbindAccessories = 41,
  NetworkAPIRequestOTA = 42,
  NetworkAPIReportOTAStatus = 43,
  NetworkAPIIotDoAction = 44,
  NetworkAPIIotGetProperty = 45,
  NetworkAPIIotSetProperty = 46,
  NetworkAPIReportPasswordList = 47,
  NetworkAPISyncBLEDeviceKeys = 48,
  NetworkAPIReportBatchEvents = 49,
  NetworkAPIEventsReport = 50,
  NetworkAPIRemoteLock = 51,
  NetworkAPIEventList = 52,
  NetworkAPIGetRequestPlayback = 53,
  NetworkAPIGetDeviceAttrs = 54,
  NetworkAPIDeviceMatchCheck = 55,
  NetworkAPIGetServerTime = 56,
  NetworkAPITemperateToken = 57,
  NetworkAPIStrangerNoteList = 58,
  NetworkAPIStrangerNote = 59,
  NetworkAPIStrangerNoteDel = 60,
  NetworkAPIPtzPresetAdd = 61,
  NetworkAPIPtzPresetList = 62,
  NetworkAPIPtzPresetUpdate = 63,
  NetworkAPIPtzPresetDel = 64,
  NetworkAPIPtzPresetReturnSet = 65,
  NetworkAPIConnDevice = 66,
  NetworkAPIDevicekeySynckey = 67,
...
SWIFT_CLASS("_TtC11CLDSmartSDK13NetworkResult")
