...
@class FGCDAsyncReadPacket;
@class FGCDAsyncWritePacket;
@class FGCDAsyncSocketPreBuffer;
@protocol FGCDAsyncSocketDelegate;

NS_ASSUME_NONNULL_BEGIN

extern NSString *const FGCDAsyncSocketException;
extern NSString *const FGCDAsyncSocketErrorDomain;

extern NSString *const FGCDAsyncSocketQueueName;
extern NSString *const FGCDAsyncSocketThreadName;

extern NSString *const FGCDAsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const FGCDAsyncSocketUseCFStreamForTLS;
#endif
#define FGCDAsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define FGCDAsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define FGCDAsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
extern NSString *const FGCDAsyncSocketSSLPeerID;
extern NSString *const FGCDAsyncSocketSSLProtocolVersionMin;
extern NSString *const FGCDAsyncSocketSSLProtocolVersionMax;
extern NSString *const FGCDAsyncSocketSSLSessionOptionFalseStart;
extern NSString *const FGCDAsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const FGCDAsyncSocketSSLCipherSuites;
extern NSString *const FGCDAsyncSocketSSLALPN;
#if !TARGET_OS_IPHONE
extern NSString *const FGCDAsyncSocketSSLDiffieHellmanParameters;
#endif

#define FGCDAsyncSocketLoggingContext 65535


typedef NS_ERROR_ENUM(FGCDAsyncSocketErrorDomain, FGCDAsyncSocketError) {
	FGCDAsyncSocketNoError = 0,           // Never used
	FGCDAsyncSocketBadConfigError,        // Invalid configuration
	FGCDAsyncSocketBadParamError,         // Invalid parameter was passed
	FGCDAsyncSocketConnectTimeoutError,   // A connect operation timed out
	FGCDAsyncSocketReadTimeoutError,      // A read operation timed out
	FGCDAsyncSocketWriteTimeoutError,     // A write operation timed out
	FGCDAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	FGCDAsyncSocketClosedError,           // The remote peer closed the connection
	FGCDAsyncSocketOtherError,            // Description provided in userInfo
};
...
@interface FGCDAsyncSocket : NSObject

/**
 * FGCDAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, FGCDAsyncSocket will automatically create it's own socket queue.
...
- (instancetype)initWithDelegate:(nullable id<FGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id<FGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq NS_DESIGNATED_INITIALIZER;

/**
 * Create FGCDAsyncSocket from already connect BSD socket file descriptor
**/
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<FGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error;

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<FGCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError **)error;

#pragma mark Configuration

@property (atomic, weak, readwrite, nullable) id<FGCDAsyncSocketDelegate> delegate;
...
- (void)getDelegate:(id<FGCDAsyncSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id<FGCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
...
**/
- (void)synchronouslySetDelegate:(nullable id<FGCDAsyncSocketDelegate>)delegate;
- (void)synchronouslySetDelegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id<FGCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;

/**
 * By default, both IPv4 and IPv6 are enabled.
 * 
 * For accepting incoming connections, this means FGCDAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means FGCDAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, FGCDAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, FGCDAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of FGCDAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a FGCDAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a FGCDAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that FGCDAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - FGCDAsyncSocketManuallyEvaluateTrust
 ...
 *     So FGCDAsyncSocket will invoke the delegate method socket:shouldTrustPeer: passing the SecTrustRef.
 *
 ...
 *
 * - FGCDAsyncSocketUseCFStreamForTLS (iOS only)
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     By default FGCDAsyncSocket will use the SecureTransport layer to perform encryption.
 ...
 *     
 *     However, if you absolutely must, you can instruct FGCDAsyncSocket to use the old-fashioned encryption
 *     technique by going through the CFStream instead. So instead of using SecureTransport, FGCDAsyncSocket
 ...
 *     
 *     Thus all the other keys in the given dictionary will be ignored by FGCDAsyncSocket,
 ...
 *
 * - FGCDAsyncSocketSSLProtocolVersionMin
 * - FGCDAsyncSocketSSLProtocolVersionMax
 ...
 *
 * - FGCDAsyncSocketSSLDiffieHellmanParameters (Mac OS X only)
 *     The value must be of type NSData.
 ...
 *     You MUST use manual trust evaluation instead (see FGCDAsyncSocketManuallyEvaluateTrust).
 ...
 *     You MUST use FGCDAsyncSocketSSLProtocolVersionMin & FGCDAsyncSocketSSLProtocolVersionMin instead.
...
/**
 * FGCDAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * This is because the FGCD API offers no mechanism to discover a queue's targetQueue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(FGCDAsyncSocket *)sock
 * {
 ...
 *     return socketQueue;
 * }
 * - (void)socket:(FGCDAsyncSocket *)sock didAcceptNewSocket:(FGCDAsyncSocket *)newSocket
 * {
 ...
 *   The faster and more powerful FGCD API isn't notified properly in this case.
 * 
 ...
 * Example usage:
 * 
 * - (void)socket:(FGCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port
 * {
...
@protocol FGCDAsyncSocketDelegate <NSObject>
@optional
...
**/
- (nullable dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(FGCDAsyncSocket *)sock;
 ...
 * The host parameter will be an IP address, not a DNS name.
 **/
- (void)socket:(FGCDAsyncSocket *)sock didConnectToUrl:(NSURL *)url;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)socket:(FGCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)socket:(FGCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(FGCDAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used for things such as updating progress bars.
**/
- (void)socket:(FGCDAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(FGCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(FGCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(FGCDAsyncSocket *)sock;
 ...
 * 
 * Note: If the FGCDAsyncSocket instance is deallocated while it is still connected,
...
**/
- (void)socketDidDisconnect:(FGCDAsyncSocket *)sock withError:(nullable NSError *)err;
...
**/
- (void)socket:(FGCDAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
